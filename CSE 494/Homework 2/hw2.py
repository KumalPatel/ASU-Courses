        ...
        for x in np.arange(pad, image_width + pad):
            for ky in range(kernel_height):
                for kx in range(kernel_width):
                    accumulator = (kernel * image[y - pad: y + pad + 1, x - pad: x + pad + 1] ).sum() # sum the region with the kernel
                    out_image[y - pad, x - pad] = accumulator # apply the kernel to the output image
...
def executeProblem2(measurements):
    ...
    # TODO: Implement the Moving Average Model here
    for index, current_measurement in enumerate(measurements): 
        state = K * current_measurement + (1-K) * state
...
def executeProblem3(measurements, sensor_variance):
    ...
    filtered[0] = xk
    # Initalize Prediction Value
    pt = np.empty(len(measurements))
    pt[0] = P
    ...
    for index, current_measurement in enumerate(measurements):
        ...
        Kt = ptt / (ptt + R)
        xk = xt + Kt*(current_measurement - xt)
        pt[index] = (1-Kt)*ptt
        filtered[index] = xk
