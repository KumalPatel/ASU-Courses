...
int main()
{
  ...
  // During first execution, the"t.txt in source directory. list.txt is generated by save() while exiting the program.
	load("list.txt");
	do {
		cout << "\nCSE240 HW10\n";
...
void changeNoOfCopies(Book* ptr, int newNum)
{
	...
	libraryType type = ptr->getLibraryType(); // define all parameters

	*ptr = Book(name, newNum, type);
}
...
{
	Container* tempList = list;				// work on a copy of 'list'
	Book* newBook; // to assign data to

	if (type == hayden)
		newBook = new Hayden(name_input, copies_input, type); // if hatden create hayden object
	else
		newBook = new Noble(name_input, copies_input, type); // else create noble object

	Container* temp = new Container();
	temp->book = newBook;

	if (tempList == NULL)// if list is empty set newnode to head
		list = temp;
	else
	{
		temp->next = tempList; // else set next node to head
		list = temp; // set head to newnode
	}
}
...
// No implementation needed here, however it may be helpful to listreview this function

void displayList()
{
	Container *tempList = list;			// work on a copy of 'list'

	while (tempList != NULL) // traverse to find last node
	{
		tempList->book->displayBook(); // display book for templist
...
{
	Container *tempList = list; // temp var for head of list
	ofstream file;
	int num = 0;

	while (tempList != NULL)
	{
		tempList = tempList->next; // traverse until last book
		num++; // count number of books
	}

	file.open(fileName);
	tempList = list; // set tempList to head of list

	file << num << endl; // write the number of books to the file

	while (tempList != NULL)
	{
		file << tempList->book->getName() << endl; // print name, copies, and library type on a new line
		file << tempList->book->getCopies() << endl;
		file << tempList->book->getLibraryType() << endl;
		tempList = tempList->next; // traverse until the last book
	}
...
{
	Container *tempList = list; // temp for head of list
	ifstream file;
	Book *newBook;
	int num = 0;
	string name;
	...
	{
		file >> num; // reads number of books and stores value into num

		for (int i = 0; i < num; i++) // loop to number of books
		{
			...
			if (type == 0)
			{
				addBook(name, copies, (libraryType)type); // if its hayden add to list of type hayden
			}

			else
			{
				addBook(name, copies, (libraryType)type); // if not add to ist of type noble
			}
		}
		file.close();
	}
