...
{
	char temp[4][50];
	int num = STRING_LENGTH-1;

	for(int i = 0; i < NUM_STRINGS; i++)
	{
		for(int j = 0; j < STRING_LENGTH; j++)
		{
			temp[i][j] = strings[i][num--];
		}
		num = STRING_LENGTH-1; // resets num after j reaches limit
	}
...
void encryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	for(int i = 0; i < NUM_STRINGS; i++)
	{
		for(int j = 0; j < STRING_LENGTH; j++)
		{
			if(strings[i][j] != '\0')
			{
				ascii = (int) strings[i][j] + key;
				strings[i][j] = (char) ascii;
			}
		}
	}
}
...
int splitAndPrintSentences(char s[NUM_STRINGS*STRING_LENGTH])
{
	...
	for(int j = 0; j < NUM_STRINGS*STRING_LENGTH; j++)
	{
		if(s[j] != '.')
		{
			str[i][k] = s[j];
...
void determinant(int matrixA[3][3])
{
	int  det = 0;
	// enter code below
	det = matrixA[0][0]*((matrixA[1][1]*matrixA[2][2])-(matrixA[2][1]*matrixA[1][2]));
	det += -matrixA[0][1]*((-matrixA[1][0]*-matrixA[2][2])-(-matrixA[2][0]*-matrixA[1][2]));
	det += matrixA[0][2]*((matrixA[1][0]*matrixA[2][1])-(matrixA[2][0]*matrixA[1][1]));
