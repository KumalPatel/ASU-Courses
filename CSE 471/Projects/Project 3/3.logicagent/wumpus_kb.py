...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for index, value in enumerate(tvec):
        if value == True:
            axiom_str += percepts[index] + str(t) + ' '
        else:
            axiom_str += '~' + percepts[index] + str(t) + ' '      
    axiom_str = axiom_str.split()
    axiom_str = ' & '.join(axiom_str)
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    walls = [( (x-1), y), (x, (y-1) ), ( (x+1), y), (x, (y+1) )]
    
    for x_value, y_value in walls:
        if (x_value >= xmin and y_value >= ymin) and (x_value <= xmax and y_value <= ymax):
            axiom_str += pit_str(x_value, y_value) + ' '
        
    axiom_str = axiom_str.split()
    axiom_str = ' | '.join(axiom_str)
    axiom_str = '{0} <=> ({1})'.format(breeze_str(x,y), axiom_str)
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for x_value, y_value in walls:
        if (x_value >= xmin and y_value >= ymin) and (x_value <= xmax and y_value <= ymax):
            axiom_str += wumpus_str(x_value, y_value) + ' '
        
    axiom_str = axiom_str.split()
    axiom_str = ' | '.join(axiom_str)
    axiom_str = '{0} <=> ({1})'.format(stench_str(x, y), axiom_str)
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    """
    axiom_str = [wumpus_str(x_value, y_value) for x_value in range(xmin, xmax + 1) for y_value in range(ymin, ymax + 1)]
    axiom_str = ' | '.join(axiom_str)
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    """
    axiom_str = ''
    possible_locations = [(x_value, y_value) for x_value in range(xmin, xmax + 1) for y_value in range(ymin, ymax + 1)]

    for current in possible_locations:
        notcurrent = ['~' + wumpus_str(x_value, y_value) for x_value in range(xmin, xmax+1) for y_value in range(ymin, ymax+1) if current[0] != x_value or current[1] != y_value]
        notcurrent = ' & '.join(notcurrent)
        axiom_str = '{0} >> {1}'.format(wumpus_str(current[0], current[1]), notcurrent)
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    possible_locations = ['~' + state_loc_str(x_value, y_value, t) for x_value in range(xmin, xmax + 1) for y_value in range(ymin, ymax + 1)]
    currentLocation = ('~L' + str(xi) + '_' + str(yi) + '_' + str(t))
    locations = [somelocation for somelocation in possible_locations if currentLocation != somelocation]

    locations = ' & '.join(locations)
    axiom_str = 'L' + '_'.join([str(xi), str(yi), str(t)]) + ' & ' + locations
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    for direction in directions:
        if heading.lower() == direction.lower():
            axiom_str += 'Heading{0}{1} & ('.format(direction, str(t))
        else:
            notHeading.append('~Heading' + direction + str(t))
...
def axiom_generator_location_OK(x, y, t):
    ...
    t := time
    """
    axiom_str = '~{0} & (~{1} | ({1} & ~{2})) <=> {3}'.format(pit_str(x, y), wumpus_str(x, y), state_wumpus_alive_str(t), state_OK_str(x, y, t))
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    t := time
    """
    axiom_str = axiom_str = '{2} >> {0} % {1}'.format(percept_breeze_str(t), breeze_str(x, y), state_loc_str(x,y,t))
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    t := time
    """
    axiom_str = '{2} >> {0} % {1}'.format(percept_stench_str(t), stench_str(x, y), state_loc_str(x,y,t))
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    if x - 1 >= xmin:
        arr.append('({0} & {1} & {2})'.format(state_loc_str(x-1, y, t), state_heading_east_str(t), action_forward_str(t)))
    if y - 1 >= ymin:
        arr.append('({0} & {1} & {2})'.format(state_loc_str(x, y-1, t), state_heading_north_str(t), action_forward_str(t)))
    if x + 1 <= xmax:
        arr.append('({0} & {1} & {2})'.format(state_loc_str(x+1, y, t), state_heading_west_str(t), action_forward_str(t)))
    if y + 1 <= ymax:
        arr.append('({0} & {1} & {2})'.format(state_loc_str(x, y+1, t), state_heading_south_str(t), action_forward_str(t)))

    arr.append('({0} & ~{1})'.format(state_loc_str(x, y, t), action_forward_str(t)))
    sucLoc = state_loc_str(x,y,t+1)    
    axiom_str = ' | '.join(arr)
    
    axiom_str = '{0} <=> ({1})'.format(sucLoc, axiom_str)
...
def axiom_generator_heading_north_ssa(t):
    ...
    """
    axiom_str = ''
    move = '({0} & (~{1} & ~{2}))'.format(state_heading_north_str(t), action_turn_right_str(t), action_turn_left_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_north_str(t+1), move, left, right)
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    """
    axiom_str = ''
    move = '({0} & (~{1} & ~{2}))'.format(state_heading_east_str(t), action_turn_right_str(t), action_turn_left_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_east_str(t+1), move, left, right)
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    """
    axiom_str = ''
    move = '({0} & (~{1} & ~{2}))'.format(state_heading_south_str(t), action_turn_right_str(t), action_turn_left_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_south_str(t+1), move, left, right)
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    """
    axiom_str = ''
    move = '({0} & (~{1} & ~{2}))'.format(state_heading_west_str(t), action_turn_right_str(t), action_turn_left_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_west_str(t+1), move, left, right)
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    axiom = []
    action = []
    for index in range(len(proposition_bases_actions)):
        for x,y in enumerate(proposition_bases_actions):
            if x != index:
                action.append('~' + y + str(t)) 
        axiom.append('{0}{1} <=> {2}'.format(proposition_bases_actions[index], str(t), ' & '.join(action)))

    axiom_str = ' & '.join(axiom)
    return axiom_str
